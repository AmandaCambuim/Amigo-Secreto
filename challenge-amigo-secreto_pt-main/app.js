alert ("Só clique em sortear nome após inserir todos os nomes!");
...
function adicionarAmigo() {
   ...
   if (!isNaN(nome) || nome === "" || amigos.includes(nome)) {
       alert ('É necessário inserir um nome válido e nÃ£o repetido.');
       limparCampo();
   } else {
       amigos.push(nome);
...
function sortearAmigo() {
   if (amigos.length > 0) {
       ...
       }, 5000); // Coloca um intervalo de 3 segundos para que o nome sorteado fique visÃ­vel.
       ...
       alterarTexto('h2','Não existem mais nomes possí­veis para serem sorteados');
       alterarTexto('p','')
    }
...
function retornarAmigos() {
   ...
   alterarTexto('h2',`Você devolveu o nome ${sorteado} para a lista de amigos disponí­veis.`)
   alterarTexto('p','')
